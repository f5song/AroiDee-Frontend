...
import { Recipe, fetchRecipes } from "@/lib/recipes/api";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
const CalendarMealPlanner: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFilteredRecipes = async () => {
      try {
        ...
        setFilteredRecipes(recipes); // อัพเดต filteredRecipes
      } catch (error) {
  ...
  const removeMeal = (mealId: number): void => {
    setMeals((prevMeals) => ({
      ...prevMeals,
      [selectedDate]: prevMeals[selectedDate].filter(
        (meal) => meal.id !== mealId
      ),
    }));
  };
  ...
  const renderCalendar = (): JSX.Element => {
    ...
    for (let i = 0; i < firstDay; i++) {
      days.push(
        <div
          key={`empty-${i}`}
          className="h-16 md:h-24 bg-gray-50 rounded-lg"
        ></div>
      );
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          key={day}
          className={`h-16 md:h-24 p-1 rounded-lg border ${
      ...
      <div className="bg-white rounded-lg shadow-sm p-2 md:p-4">
        ...
        <div className="grid grid-cols-7 gap-1 md:gap-2 mb-1 md:mb-2">
          {["Su", "M", "Tu", "W", "Th", "F", "Sa"].map((day) => (
            <div
              key={day}
              className="text-center font-medium text-gray-500 text-xs md:text-sm"
            >
              {day}
            </div>
          ))}
  ...
  const renderDayView = (): JSX.Element => {
    ...
    // Group meals by type
    const mealsByType = {
      [MEAL_TYPES.BREAKFAST]: dayMeals.filter(
        (meal) => meal.mealType === MEAL_TYPES.BREAKFAST
      ),
      [MEAL_TYPES.LUNCH]: dayMeals.filter(
        (meal) => meal.mealType === MEAL_TYPES.LUNCH
      ),
      [MEAL_TYPES.DINNER]: dayMeals.filter(
        (meal) => meal.mealType === MEAL_TYPES.DINNER
      ),
      [MEAL_TYPES.SNACK]: dayMeals.filter(
        (meal) => meal.mealType === MEAL_TYPES.SNACK
      ),
    };
    ...
    return (
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0">
          ...
          <h2 className="text-base sm:text-lg font-medium text-center w-full sm:w-auto">
            {formattedDate}
          </h2>
        </div>
                ...
                <div className="flex items-center mb-2">
                  ...
                  <div className="flex justify-between items-center w-full">
                    ...
                    >
                      ...
                      <span className="hidden xs:inline">Add Menu</span>
                    </Button>
                  </div>
                </div>
            ...
            <Tooltip>
              ...
              <TooltipContent side="bottom">
                <p className="text-xs sm:text-sm">
                  Calorie goal is synced with your profile settings
                </p>
                <Button
                  variant="link"
        ...
        <TabsContent value="statistics">
          <Card className="p-3 sm:p-6">
            <h3 className="text-lg sm:text-xl font-medium mb-2 sm:mb-4">
              Calorie Statistics for the Last 7 Days
            </h3>
        ...
        <DialogContent className="sm:max-w-lg max-w-[95vw] max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {selectedMealType === MEAL_TYPES.BREAKFAST
                ? " Breakfast"
                : selectedMealType === MEAL_TYPES.LUNCH
                ? " Lunch"
